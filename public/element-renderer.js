 ...
 *
 * Capabilities:
 * - URL Parameter parsing (?symbol=C, ?z=6)
 * - Navigation between all 118 elements (Prev / Next / Arrow keys)
 * - Three.js 3D Bohr Quantum Atom & Electron Cloud Simulation
 * - 3Dmol.js WebGL Interactive Molecular Structure Viewer (Ball & Stick, VDW, Wireframe)
 * - IUPAC Chemical Dossier Population (Quantum, Thermodynamic, Nuclear, Structural)
 * - KaTeX / HTML Formula Formatting
 */
...
class ElementDossierRenderer {
  constructor() {
    ...
    this.molCurrentStyle = 'stick'; // 'stick', 'sphere', 'line'

    this.init();
  }

  /**
   * Initializes page lifecycle
   */
  async init() {
    await this.loadData();
  ...
  /**
   * Parse target element from URL parameters or hash
   */
  ...
  /**
   * Update browser title, meta tags, and breadcrumbs
   */
  renderHeaderAndMeta() {
  ...
  /**
   * Formats atomic weight per IUPAC standards
   */
  ...
  /**
   * Helper to format superscript electron configuration numbers
   */
  formatConfigSuperscripts(configStr) {
    if (!configStr) return 'N/A';
    // Replace superscript numerals with HTML <sup> tags or unicode superscripts
    return configStr.replace(/\^([0-9]+)/g, '<sup>$1</sup>');
  }

  /**
   * Helper to format chemical formula subscripts (e.g. H2O -> H<sub>2</sub>O)
   */
  ...
  renderHeroSection() {
    ...
    if (nucStats) {
      ...
      const neutrons = Math.round((typeof el.atomic_mass === 'number' ? el.atomic_mass : protons) - protons);
  ...
  /**
   * SECTION 2: QUICK FACTS & QUANTUM MECHANICS
   */
  renderQuickFacts() {
    const el = this.currentElement;
    if (!el) return;
    ...
    if (enegVal && enegBar) {
      if (el.electronegativity_pauling) {
        ...
        // Pauling range approx 0.7 to 4.0
    ...
    if (ionVal && ionBar) {
      if (el.ionization_energy_first_kj_mol) {
        ...
        // Ionization range approx 380 to 2372 (He)
    ...
    // Atomic Radius & Crystal Structure
  ...
  /**
   * SECTION 3: PHYSICAL & THERMODYNAMIC PROPERTIES
   */
  ...
  /**
   * SECTION 1 (RIGHT): INTERACTIVE 3D BOHR ATOM / QUANTUM CLOUD (THREE.JS)
   */
  initBohrSimulation() {
    ...
    // Renderer with high pixel ratio
    ...
    // Generate dense nucleus cluster of protons & neutrons
    ...
    const totalNucleons = Math.min(60, protons + neutrons); // Cap visual density for performance
    ...
    for (let i = 0; i < totalNucleons; i++) {
      ...
      // Random packed sphere coordinates
    ...
    // Concentric Electron Shells & Orbiting Electrons
    ...
    shells.forEach((electronCount, shellIndex) => {
      ...
      for (let e = 0; e < electronCount; e++) {
        ...
        electronMeshes.push({
          ...
          shellRadius: radius,
          baseAngle: baseAngle,
          speed: (0.02 / (shellIndex + 1)), // Higher shells orbit at lower angular velocities
          angle: baseAngle,
    ...
    // Electron Probability Cloud (Alternative Quantum Mode)
    ...
    for (let i = 0; i < cloudCount; i++) {
      // Gaussian distribution around nucleus
      const u = Math.random();
  ...
  /**
   * SECTION 4: 3DMOL.JS INTERACTIVE COMPOUND DATABASE
   */
  initCompoundDatabase() {
    ...
    // If element has compounds, use them; otherwise provide default crystal lattice/oxide complex
  ...
  /**
   * Generates fallback coordination complex or crystal unit for elements without bespoke compounds
   */
  ...
  /**
   * Loads selected compound into 3Dmol viewer and populates dossier
   */
  loadCompound(cmp) {
  ...
  /**
   * Initializes or updates the 3Dmol.js WebGL viewport
   */
  render3DMol(xyzData) {
    ...
    // Clean existing viewer or create new one
    viewerDiv.innerHTML = '';
  ...
  /**
   * Applies the active visual style to 3Dmol.js model
   */
  applyMolStyle() {
    if (!this.molViewer) return;

    if (this.molCurrentStyle === 'stick') {
      // High-clarity Ball-and-Stick
  ...
  /**
   * Keyboard shortcuts & navigation listeners
   */
